...
#The limit on resources for this compute
VCPUS = 100
MEMORY_IN_MBS = 88192
DISK_IN_GB = 500
...
def reset_nodes():
    global _AZURE_NODES
    _AZURE_NODES = [CONF.host]


class AzureDriver(driver.ComputeDriver):
    """Azure for compute drivers.
    """
    ...
    def __init__(self, virtapi, read_only=False):
        ...
        subscription_id = CONF.azuredriver.subscription_id
        cert_file = CONF.azuredriver.cert_file
        curr_dir = os.path.dirname(os.path.realpath(__file__))
        cert_file_path = '/'.join((curr_dir, cert_file))

        if not os.path.exists(cert_file_path):
            err_msg = "Cert file '%s' does not exists!" % cert_file_path
            raise Exception(err_msg)

        self.azure_sms = AzureServicesManager(subscription_id, cert_file_path)

        if not '_AZURE_NODES' in globals():
            set_nodes([CONF.host])
    ...
    def reboot(self, context, instance, network_info, reboot_type,
        ...
        if reboot_type == 'SOFT':
            self.azure_sms.soft_reboot(cl_name, vm_name)

        elif reboot_type == 'HARD':
            self.azure_sms.hard_reboot(cl_name, vm_name)
    ...
    def attach_volume(self, context, connection_info, instance, mountpoint,
        ...
        lun = self.azure_sms.get_available_lun(service_name, vm_name)
        volume_id = connection_info['data']['volume_id']

        self.azure_sms.attach_volume(service_name, vm_name, 5, lun)
    ...
    def detach_volume(self, connection_info, instance, mountpoint,
        ...
        self.azure_sms.detach_volume(service_name, vm_name, lun)
    ...
    def detach_interface(self, instance, vif):
        ...
        try:
            ...
            raise exception.InterfaceDetachFailed('Interface is not attached')
    ...
    def unpause(self, instance):
        ...
        """
        # TODO(Vek): Need to pass context in for access to auth_token
        self.power_on(context=None, instance=instance,
    ...
    def soft_delete(self, instance):
        ...
        :param instance: nova.objects.instance.Instance
        """
        self.destroy(context=None, instance=instance, network_info=None,
    ...
    def get_available_resource(self, nodename):
        ...
        """
        if nodename not in _AZURE_NODES:
            return {}
    ...
    def get_host_stats(self, refresh=False):
        ...
        for node in _AZURE_NODES:
            host = self.host_status_base.copy()
            host['hypervisor_hostname'] = node
            host['host_hostname'] = node
            host['host_name_label'] = node
            host['hypervisor_tpye'] = 'Azure'
            host['vcpus'] = VCPUS
            host['memory_mb'] = MEMORY_IN_MBS
            host['local_gb'] = DISK_IN_GB

            host_stats.append(host)
